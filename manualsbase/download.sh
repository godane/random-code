...
for i in $(seq -w 000 999); do
	...
	if [ ! -f $file ]; then
	pdfurl="$(curl -s http://www.manualsbase.com/manual/${k}${i}/1/1/1/ | grep -a pdf | sed 's|.*downloadmanual||g' | sed 's|.*#../|http://www.manualsbase.com/|g'  | sed 's|pdf".*|pdf|g' | grep -a ^http | head -1 | sed 's|#39;||g' | sed 's|&||g')"
	echo "$pdfurl"
	echo "${k}${i}"
