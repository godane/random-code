...
;while ControlGetText($hwnd,"","[CLASS:_AOL_Edit; INSTANCE:1]") <> "aol://4401:4367:"&$area
	ControlSetText($hwnd,"","[CLASS:_AOL_Edit; INSTANCE:1]","")
	ControlSend($hwnd,"","[CLASS:_AOL_Edit; INSTANCE:1]","aol://4400:"&$area&"{DELETE}")
	;ControlSend($hwnd,"","[CLASS:_AOL_Edit; INSTANCE:1]","aol://4401:4367:"&$area&"{DELETE}")
WEnd
...
Else
	...
	if not FileExists("C:\AOLDLs\"&MakeValidFilename($title)) then
		...
		FileWrite("C:\AOLDLs\"&MakeValidFilename($title)&"\url.txt","aol://4400:"&$area)
	     ;FileWrite("C:\AOLDLs\"&MakeValidFilename($title)&"\url.txt","aol://4401:4367:"&$area)
	EndIf
	...
	;if FileExists("C:\AOLDLs\"&MakeValidFilename($title)&"\"&MakeValidFilename($subj)&"\info.txt") then
	FileWrite("C:\AOLDLs\"&MakeValidFilename($title)&"\"&MakeValidFilename($subj)&"\info.txt",$ctrl)
    ;EndIf
	;if not FileExists("C:\AOLDLs\"&MakeValidFilename($title)&"\"&MakeValidFilename($subj)&"\info.txt") Then
	 ;  DirCreate("C:\AOLDLs\"&MakeValidFilename($title)&"\"&MakeValidFilename($subj))
	 ;  FileWrite("C:\AOLDLs\"&MakeValidFilename($title)&"\"&MakeValidFilename($subj)&"\info.txt",$ctrl)
	;endif
